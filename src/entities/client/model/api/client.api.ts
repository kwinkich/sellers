...
export const ClientsAPI = {
  ...
  addClientLicenses: (id: number, licenseData: AddLicensesRequest) =>
    API.post(`clients/${id}/licenses/add`, { json: licenseData }).json<
      GApiResponse<LicenseInfo[]>
    >(),

  removeClientLicenses: (id: number, licenseIds: number[]) =>
    API.delete(`clients/${id}/licenses/remove`, {
      json: { licenseIds },
    }).json<
      GApiResponse<{
        removedCount: number;
        revokedMopUsers: number;
      }>
    >(),
...
export const clientsMutationOptions = {
  ...
  removeLicenses: () => ({
    mutationFn: ({ id, licenseIds }: { id: number; licenseIds: number[] }) =>
      ClientsAPI.removeClientLicenses(id, licenseIds),
  }),
