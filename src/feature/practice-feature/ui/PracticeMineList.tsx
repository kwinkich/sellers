...
import { PracticeMineCard } from "./PracticeMineCard";

interface Props {
  searchQuery?: string;
}

export const PracticeMineList = ({ searchQuery = "" }: Props) => {
  const {
    items: practices,
    isLoading,
    isError,
    ...
    queryKey: ["practices", "mine"],
    queryFn: (page, limit) => PracticesAPI.getMyPractices({ page, limit }),
    limit: 20,
  });
